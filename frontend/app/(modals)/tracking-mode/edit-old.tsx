...
export default function EditTrackingModeScreen() {
    ...
    const { trackingModes, updateTrackingMode, isTracking, trackingModeId } = useTracking() as any;
    ...
    const handleSave = async () => {
        if (!modeId) return;
        if (isTracking && trackingModeId === modeId) {
            Alert.alert('無法儲存', '此報平安正在執行中，請先停止報平安後再編輯。');
            return;
        }
        try {
            ...
            <TouchableWithoutFeedback onPress={Keyboard.dismiss} accessible={false}>
                <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scrollContainer}>
                    <View style={styles.header}>
                        ...
                        <Text style={styles.title}>編輯報平安</Text>
                    </View>
                    ...
                    <View style={styles.fieldGroup}>
                        ...
                        <TouchableOpacity
                            style={[styles.saveBtn, { backgroundColor: Theme.uiParameters.buttons.setting.edit }]}
                            onPress={() => {
                                if (!modeId) return;
...
const styles = StyleSheet.create({
    ...
    saveBtn: {
        marginTop: 12,
        backgroundColor: Theme.uiParameters.buttons.setting.save,
        borderRadius: Theme.radii.xl,
        paddingVertical: 16,
        alignItems: 'center',
        shadowColor: '#000',
